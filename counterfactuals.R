# What if? Software for Evaluating Counterfactuals (Stoll, King and Zeng, 2010)
...
# Not all counterfactuals can be evaluated. As the authors say:
...
# WhatIf is a package that helps to indentify those bad counterfactuals
# with a matching software. It checks if the counterfactual data are in convex hull

# Let's show an example below:
library(WhatIf) # the package itself
library(MatchIt) # for matching (not necessary, just to test other things)
library(Zelig) # for estimation
...
model1 <- zelig(vote ~ race1 + age + educate + income, data = turnout,
                model = "logit")
summary(model1)
...
# Or more generally:
# Create a new dataset with the quantities of interest of the 
# covariates (no intercept!)
