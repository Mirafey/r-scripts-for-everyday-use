...
# over non-experimental data. Amogst others, Gelman and Hill (2003), and 
# Gary King et al (2011) have talked about it. Several articles use
...
# Now we shall estimate the effect of race on voter turnout, trying to find pairs
...
match2 <- matchit(race1 ~ age + educate + income, data = turnout,  method = "nearest",
                  discard = "hull.both", # discard both control and treated observations
                  # that are outside of the convex hull 
                  # See: http://r.iq.harvard.edu/docs/matchit/2.4-20/matchit.pdf pp.24
                  replace = TRUE, # observations can be matched many times
                  ratio = 2) # number of control units matched with each treated unit
...
# Seving the object into a data frame
...
model1 <- zelig(vote ~ race1 + age + educate + income, data = m.data, model = "logit")
summary(model1)
